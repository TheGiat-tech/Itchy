...
function getValidImage(imgUrl: string | undefined, index: number): string {
...
function normalizePest(pest: any): PestCardData {
  const frontmatter = pest?.frontmatter ?? {};

  return {
    slug: pest?.slug ?? "",
    title: frontmatter.title || frontmatter.titleHebrew || "כתבה ללא כותרת",
    titleLatin: frontmatter.titleLatin,
    category: frontmatter.category,
    pestType: frontmatter.pestType,
    date: frontmatter.date,
    lastUpdated: frontmatter.updatedAt || frontmatter.lastUpdated,
    image:
    ...
    identification: frontmatter.identification,
    subtitle: frontmatter.subtitle,
    content: pest?.content || "",
  };
}

function getPostExcerpt(post: PestCardData, defaultText: string): string {
  const rawText = post.content || post.identification || post.subtitle || "";
...
export default async function HomePage() {
  const allPests = (getAllPests() || []).map(normalizePest);

  const heroPost = allPests[0] || null;
  const latestPosts = allPests.slice(1, 5);

  let invasivePests = allPests
    .filter((p) => p.category === "מינים פולשים" || p.pestType === "פולש")
    .slice(0, 4);
              ...
              <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                <div className="relative h-64 sm:h-96 w-full bg-gray-100">
                  <Image
                    ...
                    alt={heroPost.title || "איצ'י מזיקים"}
                    fill
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs text-gray-400">
                    {heroPost.date || heroPost.lastUpdated ? (
                  ...
                  latestPosts.map((post, idx) => (
                    <div key={post.slug} className="flex gap-4 bg-white p-3 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                      <div className="relative w-20 h-20 flex-shrink-0 rounded-md overflow-hidden bg-gray-50">
                        <Image src={getValidImage(post.image, idx + 1)} alt={post.title} fill className="object-cover" />
                      </div>
